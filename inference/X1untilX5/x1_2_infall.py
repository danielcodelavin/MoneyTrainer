...
class SingleStockDataset(Dataset):
    def __init__(self, stock_data, topic_vector):
        # Expect stock_data and topic_vector to already have batch dimension
        self.stock_data = stock_data.squeeze(0)  # Remove batch dim
        self.topic_vector = topic_vector.squeeze(0)  # Remove batch dim
    
    def __len__(self):
        return 1
    
    def __getitem__(self, idx):
        return self.stock_data, self.topic_vector
...
def check_batchnorm_stats(model):
    ...
    for name, module in model.named_modules():
        if isinstance(module, nn.BatchNorm1d):
            ...
            print(f"Running mean (first 5): {module.running_mean[:5].cpu().numpy()}")
            print(f"Running var (first 5): {module.running_var[:5].cpu().numpy()}")
...
def main():
    ...
    # Load models and check BatchNorm state
    model = MultiModalMLP(device=device)
    print("\nBatchNorm state before loading checkpoint:")
    check_batchnorm_stats(model)
    
    checkpoint = torch.load(checkpoint_path)
    model.load_state_dict(checkpoint['model_state_dict'])
    
    print("\nBatchNorm state after loading checkpoint:")
    check_batchnorm_stats(model)
    
    print("\nSetting model to eval mode...")
    model.eval()
    
    print("\nBatchNorm state after eval():")
    check_batchnorm_stats(model)
    ...
    with open(results_file_path, 'w', newline='') as csvfile:
        ...
        writer.writerow(['symbol', 'result', 'percent', 'features_mean', 'features_std'])
    
    print(f"Created results file at: {results_file_path}")
    ...
    with torch.no_grad():
        for i, stock in enumerate(stock_data):
            print(f"\nProcessing {stock['symbol']} ({i+1}/{len(stock_data)})")
            try:
                ...
                if data_result is not None:
                    ...
                    all_topic_tensors.append(topic_tensor)
                    all_symbols.append(stock['symbol'])
                    
                    # Process batch when we have batch_size stocks
                    if len(all_symbols) == batch_size:
                        ...
                        try:
                            # Create batch
                            batch_stock = torch.stack(all_stock_tensors).to(device)
                            batch_topic = torch.stack(all_topic_tensors).to(device)
                            
                            print(f"Batch shapes - Stock: {batch_stock.shape}, Topic: {batch_topic.shape}")
                            print(f"Batch stock stats - Mean: {batch_stock.mean():.4f}, Std: {batch_stock.std():.4f}")
                            print(f"Batch topic stats - Mean: {batch_topic.mean():.4f}, Std: {batch_topic.std():.4f}")
                            
                            # Track feature statistics through model
                            stock_features = model.stock_path(batch_stock)
                            topic_features = model.topic_path(batch_topic)
                            combined = torch.cat((stock_features, topic_features), dim=1)
                            print(f"Combined features stats - Mean: {combined.mean():.4f}, Std: {combined.std():.4f}")
                            
                            # Get predictions
                            predictions = model.combined_path(combined)
                            
                            # Save each prediction with feature statistics
                            for idx, (symbol, pred) in enumerate(zip(all_symbols, predictions)):
                                result = pred.item()
                                # Get feature statistics for this sample
                                sample_features = combined[idx]
                                features_mean = sample_features.mean().item()
                                features_std = sample_features.std().item()
                                
                                with open(results_file_path, 'a', newline='') as csvfile:
                                    writer = csv.writer(csvfile)
                                    writer.writerow([symbol, result, f"{result * 100:.2f}%", 
                                                   f"{features_mean:.4f}", f"{features_std:.4f}"])
                                print(f"Processed {symbol}: {result:.4f} (features mean: {features_mean:.4f}, std: {features_std:.4f})")
                ...
                print(f"Error collecting data for {stock['symbol']}: {str(e)}")
                continue
    
        # Process remaining stocks if any
        if len(all_symbols) > 0:
            print(f"\nProcessing final batch of {len(all_symbols)} stocks")
            try:
                batch_stock = torch.stack(all_stock_tensors).to(device)
                batch_topic = torch.stack(all_topic_tensors).to(device)
    ...
    # Final BatchNorm state check
    print("\nFinal BatchNorm state:")
    check_batchnorm_stats(model)
