...
def analyze_by_factor(df: pd.DataFrame, info_df: pd.DataFrame, factor: str) -> Tuple[dict, dict]:
    """
    Analyze stock performance grouped by different factors (Market Cap, Volume, Sector).
    
    Args:
        df: DataFrame with prediction results
        info_df: DataFrame with stock information
        factor: The factor to analyze by ('Market Cap', 'Volume', 'Sector')
    
    Returns:
        Tuple of (results dictionary, stock listings dictionary)
    """
    results = defaultdict(lambda: defaultdict(list))
    stock_listings = defaultdict(list)
    
    # Merge dataframes
    merged_df = df.merge(info_df, left_on='symbol', right_on='Symbol', how='left')
    ...
    for name, group in merged_df.groupby('Group'):
        ...
        try:
            ...
            predicted_positives = len(group[group['result'] > 0])
...
def generate_analysis_report(results: dict, stock_listings: dict, output_path: str):
    """
    Generate a detailed analysis report from the results and stock listings.
    
    Args:
        results: Dictionary containing analysis results by factor
        stock_listings: Dictionary containing stock listings by category
    ...
    with open(output_path, 'w') as f:
        ...
        for factor, factor_results in results.items():
            ...
            # Sort results by L1 distance (lower is better)
        ...
        # Add detailed stock listings at the end
        ...
        for factor, factor_listings in stock_listings.items():
            ...
            for category, stocks in factor_listings.items():
                ...
                # Sort stocks by symbol if available
                try:
...
def process_csv(filepath: str, info_filepath: str) -> Tuple[pd.DataFrame, dict, dict]:
    ...
    for idx, row in df.iterrows():
        ...
        if stock_data_result is not None:
            ...
            gt_value = returngroundtruthstock(row['symbol'], next_date)
            if gt_value.numel() == 0:
                print(f"No ground truth for {row['symbol']} on {next_date}")
                continue
            ...
            df.at[idx, 'prediction_error'] = abs(percentage_change - row['result'])
            
            l1_dist = float(abs(percentage_change - row['result']))
            l1_distances.append(l1_dist)
            
            true_values.append(percentage_change)
            predicted_values.append(float(row['result']))
    ...
    if true_values and predicted_values:
        ...
        factor_analysis = {}
        stock_listings = {}
        for factor in ['Market Cap', 'Volume', 'Sector']:  # Removed 'Industry'
...
def main():
    filepath = "/Users/daniellavin/Desktop/proj/MoneyTrainer/results/2024/12/02/results_20241202_130000.csv"
