...
class MultiModalMLP(nn.Module):
    ...
    def forward(self, x):
        # Extract features from integrated tensor
        # x shape: [batch_size, 111] where:
        # - Position 0 is dummy GT
        # - Positions 1:81 are stock data
        # - Positions 81:111 are topic vector
        stock_data = x[:, 1:81]
        topic_vector = x[:, 81:]
...
def datagenerator(
    ...
    try:
        ...
        # Create integrated tensor matching training data structure
        integrated_tensor = torch.cat([
            torch.tensor([0.0], dtype=torch.float32),  # Dummy GT value in position 0
            stock_data,                                # Stock data positions 1:81
            news_embedding                             # Topic data positions 81:111
        ])
...
def main():
    ...
    with torch.no_grad():
        for i, stock in enumerate(stock_data):
            ...
            try:
                integrated_tensor = datagenerator(
                ...
                if integrated_tensor is not None:
                    all_tensors.append(integrated_tensor)
                    ...
                    if len(all_symbols) == batch_size:
                        ...
                        for symbol, pred in zip(all_symbols, predictions):
                            result = pred.item()
