...
class FinancialDataset(Dataset):
    def __init__(self, data_path):
        """
        Dataset class for loading financial tensors with 30-topic vectors
        Each tensor has shape (111,) where:
...
class WeightedEnsembleModel(nn.Module):
    ...
    def forward(self, stock_data, topic_vector, ground_truth=None):
        ...
        if self.training:
            return predictions_stack.transpose(0, 1), confidences_stack.transpose(0, 1)  # [num_models, batch_size, 1]
        else:
            model_weights = self.calculate_model_weights()
            weighted_pred = torch.sum(
                predictions_stack * model_weights.view(1, -1, 1),  # [batch_size, num_models, 1]
                dim=1
            )
...
def train_ensemble_model(
    ...
    for epoch in range(start_epoch, params['num_epochs']):
        ...
        for batch in tqdm(train_loader, desc=f"Epoch {epoch+1}/{params['num_epochs']}", leave=True):
...
if __name__ == "__main__":
    params = {
        'num_epochs': 300,
        'batch_size': 128,
        'num_models': 7,  # Updated to 7 models
    ...
    train_ensemble_model(
        ...
        checkpoint_dir="/Users/daniellavin/Desktop/proj/MoneyTrainer/TANDEM/XT/check",
        report_path="/Users/daniellavin/Desktop/proj/MoneyTrainer/TANDEM/XT/check/1TX_report.txt",
        params=params
    )